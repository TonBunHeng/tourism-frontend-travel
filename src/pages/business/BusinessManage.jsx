  ...
  if (loading) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-sm font-medium text-gray-500">Loading business manager...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 dark:bg-zinc-950 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-gray-200 dark:border-zinc-800 shadow-sm">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <Link to="/business/dashboard" className="text-xs font-bold text-gray-500 hover:text-gray-900 flex items-center gap-1">
                <ArrowLeft className="w-3.5 h-3.5" /> Dashboard
              </Link>
              <span className="text-gray-300">/</span>
              <span className="text-xs font-bold text-[#003E83] dark:text-blue-400 capitalize">{business.verification_status}</span>
            </div>
            <h1 className="text-2xl font-extrabold text-gray-900 dark:text-white">{business.name}</h1>
          </div>
          ...
          <div className="flex items-center gap-2">
            <Link
              ...
              target="_blank"
              className="px-3.5 py-2 bg-gray-100 dark:bg-zinc-800 hover:bg-gray-200 text-gray-700 dark:text-zinc-200 text-xs font-bold rounded-xl flex items-center gap-1.5"
            >
              <ExternalLink className="w-3.5 h-3.5" /> Public View
            </Link>
            <Link
              to={`/business/businesses/${business.id}/edit`}
              className="px-4 py-2 bg-[#003E83] hover:bg-[#002e62] text-white text-xs font-bold rounded-xl shadow-xs flex items-center gap-1.5"
            >
        ...
        <div className="p-4 rounded-2xl bg-blue-50 dark:bg-zinc-900 border border-blue-200 dark:border-zinc-800 flex items-center justify-between gap-4 text-xs">
          <div className="flex items-center gap-3">
            <CheckCircle className="w-5 h-5 text-blue-600 shrink-0" />
            <div>
              <span className="font-bold text-gray-900 dark:text-white">Verification Lifecycle Status: </span>
              <span className="font-extrabold text-blue-700 dark:text-blue-400 uppercase">{business.verification_status}</span>
            </div>
          </div>
        ...
        <div>
          ...
          {activeTab === 'overview' && (
            <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-gray-200 dark:border-zinc-800 space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-4 border-t border-gray-100 dark:border-zinc-800 text-xs">
                ...
                <div>
                  <span className="text-gray-400 block font-semibold">Province</span>
                ...
                <div>
                  <span className="text-gray-400 block font-semibold">Price Range</span>
          ...
          {activeTab === 'services' && (
            <div className="space-y-6">
              <form onSubmit={handleAddService} className="bg-white dark:bg-zinc-900 p-5 rounded-2xl border border-gray-200 dark:border-zinc-800 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <input
                    ...
                    className="p-2.5 bg-gray-50 dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-xl text-xs font-semibold text-gray-900 dark:text-white"
                    required
                  />
                  <input
                    type="number"
                ...
                />
                <button type="submit" className="px-4 py-2 bg-[#003E83] text-white text-xs font-bold rounded-xl cursor-pointer">
                  Add Service
                </button>
              </form>

              <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-gray-200 dark:border-zinc-800 space-y-3">
                <h4 className="text-sm font-bold text-gray-900 dark:text-white">Configured Services ({services.length})</h4>
                {services.map((s) => (
                  <div key={s.id} className="p-3.5 bg-gray-50 dark:bg-zinc-800/60 rounded-xl border border-gray-100 dark:border-zinc-700 flex items-center justify-between gap-3">
                    <div>
                    ...
                    <div className="flex items-center gap-3">
                      {s.price && <span className="font-extrabold text-xs text-emerald-600">${s.price}</span>}
                      <button onClick={() => handleDeleteService(s.id)} className="text-rose-500 hover:text-rose-700 p-1 cursor-pointer">
          ...
          {activeTab === 'hours' && (
            <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-gray-200 dark:border-zinc-800 space-y-4">
              <div className="flex items-center justify-between border-b border-gray-100 dark:border-zinc-800 pb-3">
                <h4 className="text-sm font-bold text-gray-900 dark:text-white">Weekly Schedule Editor</h4>
                <button onClick={handleSaveHours} className="px-4 py-2 bg-[#003E83] text-white text-xs font-bold rounded-xl flex items-center gap-1.5 cursor-pointer">
              ...
              <div className="space-y-3">
                {hours.map((h, idx) => (
                  <div key={h.day_of_week} className="p-3 bg-gray-50 dark:bg-zinc-800/60 rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                      ...
                      {!h.is_closed && (
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            className="p-1.5 bg-white dark:bg-zinc-700 border border-gray-200 dark:border-zinc-600 rounded-lg text-xs font-semibold"
                          />
          ...
          {activeTab === 'gallery' && (
            <div className="space-y-6">
              <form onSubmit={handleAddImage} className="bg-white dark:bg-zinc-900 p-5 rounded-2xl border border-gray-200 dark:border-zinc-800 space-y-3">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 p-2.5 bg-gray-50 dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-xl text-xs font-semibold text-gray-900 dark:text-white"
                  />
                  <button type="submit" className="px-4 py-2.5 bg-[#003E83] text-white text-xs font-bold rounded-xl cursor-pointer">
                    Upload
                  </button>
                </div>
              </form>
              ...
              <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-gray-200 dark:border-zinc-800 space-y-3">
                <h4 className="text-sm font-bold text-gray-900 dark:text-white">Gallery Photos ({gallery.length})</h4>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {gallery.map((g) => (
                    <div key={g.id} className="relative aspect-square rounded-xl overflow-hidden group bg-gray-100">
          ...
          {activeTab === 'promotions' && (
            <div className="space-y-6">
              <form onSubmit={handleAddPromotion} className="bg-white dark:bg-zinc-900 p-5 rounded-2xl border border-gray-200 dark:border-zinc-800 space-y-3">
              ...
              <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-gray-200 dark:border-zinc-800 space-y-3">
                <h4 className="text-sm font-bold text-gray-900 dark:text-white">Active Promotions ({promotions.length})</h4>
                {promotions.map((p) => (
                  <div key={p.id} className="p-4 bg-amber-50/50 dark:bg-amber-950/20 rounded-xl border border-amber-200 dark:border-amber-900 flex items-center justify-between gap-3">
                    <div>
                    ...
                    <button onClick={() => handleDeletePromotion(p.id)} className="text-rose-500 hover:text-rose-700 p-1 cursor-pointer">
                      <Trash2 className="w-4 h-4" />
                    </button>
                  </div>
                ))}
          ...
          {activeTab === 'events' && (
            <div className="space-y-6">
              <form onSubmit={handleAddEvent} className="bg-white dark:bg-zinc-900 p-5 rounded-2xl border border-gray-200 dark:border-zinc-800 space-y-3">
              ...
              <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-gray-200 dark:border-zinc-800 space-y-3">
                <h4 className="text-sm font-bold text-gray-900 dark:text-white">Business Events ({events.length})</h4>
                {events.map((e) => (
                  <div key={e.id} className="p-4 bg-gray-50 dark:bg-zinc-800/60 rounded-xl border border-gray-200 dark:border-zinc-700 space-y-1">
                    <h5 className="font-bold text-xs text-gray-900 dark:text-white">{e.title}</h5>
                    {e.description && <p className="text-[11px] text-gray-500">{e.description}</p>}
                    <span className="text-[10px] text-gray-400 block">{e.start_date}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {activeTab === 'reviews' && (
            <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-gray-200 dark:border-zinc-800 space-y-4">
              <h4 className="text-sm font-bold text-gray-900 dark:text-white">Customer Feedback & Replies ({reviews.length})</h4>
              {reviews.length === 0 ? (
                  ...
                  {reviews.map((r) => (
                    <div key={r.id} className="pt-4 space-y-2">
                      ...
                      {r.replies && r.replies.map((reply) => (
                        <div key={reply.id} className="p-3 bg-blue-50 dark:bg-zinc-800 rounded-xl text-xs space-y-1">
                          <span className="font-bold text-[#003E83] dark:text-blue-400">Your Reply:</span>
                      ...
                      <div className="flex items-center gap-2 pt-2">
                        <input
                          ...
                          className="flex-1 p-2 bg-gray-50 dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-xl text-xs text-gray-900 dark:text-white"
                        />
                        <button
                          onClick={() => handleReplySubmit(r.id)}
                          className="px-3.5 py-2 bg-[#003E83] text-white text-xs font-bold rounded-xl cursor-pointer"
                        >
                          Reply
                        </button>
                      </div>
