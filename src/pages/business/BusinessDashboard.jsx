  ...
  if (loading) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-sm font-medium text-gray-500 dark:text-zinc-400">Loading Business Dashboard...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 dark:bg-zinc-950 py-6 px-4 sm:px-6 lg:px-8 transition-colors">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 border-b border-gray-200 dark:border-zinc-800">
          ...
          <div className="flex items-center gap-2">
            <Link
              ...
              className="px-3.5 py-1.5 bg-gray-100 dark:bg-zinc-800 hover:bg-gray-200 dark:hover:bg-zinc-700 text-gray-700 dark:text-zinc-200 text-xs font-semibold rounded-md border border-gray-300 dark:border-zinc-700 transition-colors flex items-center gap-1.5"
            >
            ...
            <Link
              ...
              className="px-3.5 py-1.5 bg-[#003E83] hover:bg-[#002e62] dark:bg-[#60a5fa] dark:hover:bg-[#3b82f6] dark:text-zinc-950 text-white text-xs font-semibold rounded-md shadow-xs transition-colors flex items-center gap-1.5 cursor-pointer"
            >
        ...
        {/* Verification Alert */}
        {rejectedCount > 0 && (
          <div className="bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-900 p-3.5 rounded-lg flex items-center justify-between gap-3 text-xs">
            <div className="flex items-center gap-2 text-rose-700 dark:text-rose-300">
              <AlertCircle className="w-4 h-4 shrink-0 text-rose-500" />
              <span>You have {rejectedCount} submission(s) requiring edits before admin verification.</span>
            </div>
            <Link to="/business/businesses" className="font-semibold underline text-rose-800 dark:text-rose-200">Review</Link>
          </div>
        )}

        {/* Key Metrics Grid */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          <div className="bg-white dark:bg-zinc-900 p-4 rounded-lg border border-gray-200 dark:border-zinc-800 shadow-xs space-y-1">
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-lg border border-gray-200 dark:border-zinc-800 p-5 space-y-4 shadow-xs">
          ...
          {businesses.length === 0 ? (
            <div className="py-10 text-center space-y-2">
              ...
              <Link
                to="/business/businesses/new"
                className="inline-block px-4 py-2 bg-[#003E83] text-white text-xs font-semibold rounded-md shadow-xs"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {businesses.map((biz) => {
                ...
                return (
                  <div
                    key={biz.id}
                    className="p-4 rounded-lg border border-gray-200 dark:border-zinc-800 bg-gray-50/50 dark:bg-zinc-800/40 flex flex-col justify-between gap-3 hover:border-[#003E83] dark:hover:border-blue-500 transition-colors"
                  >
                    ...
                    <div className="pt-2.5 border-t border-gray-200/80 dark:border-zinc-700/60 flex items-center justify-between">
                      <span className="text-xs text-gray-500 font-medium">{biz.category?.name || 'Tourism'}</span>
                      <Link
                        to={`/business/businesses/${biz.id}`}
                        className="px-3 py-1.5 bg-[#003E83] hover:bg-[#002e62] dark:bg-[#60a5fa] dark:hover:bg-[#3b82f6] dark:text-zinc-950 text-white text-xs font-semibold rounded-md shadow-xs transition-colors flex items-center gap-1"
                      >
